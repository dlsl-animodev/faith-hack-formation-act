...
export async function POST(request: Request) {
  ...
  if (!verifyAdminRequest(request)) {
    console.log("[API /events/create] ========== AUTHORIZATION FAILED ==========");
    return jsonErr("Unauthorized", 401);
  }
  
  console.log("[API /events/create] ========== AUTHORIZATION SUCCESS ==========");

  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000";
  const supabase = createAdminClient();

  console.log("[API /events/create] Checking for existing active event code in app_state");
  ...
  if (activeCode) {
    console.log("[API /events/create] Active code found:", activeCode, "- checking if event is still active");
    ...
    if (currentEv?.is_active) {
      console.log("[API /events/create] Event is still active, marking it as inactive");
    ...
    console.log("[API /events/create] Clearing inactive event code from app_state");
    await supabase
      .from("app_state")
      .update({
  ...
  if (orphanActive) {
    console.log("[API /events/create] Found orphaned active event:", orphanActive.event_code, "- marking as inactive");
    await supabase
      .from("events")
      .update({
        is_active: false,
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    console.log("[API /events/create] Event code collision, regenerated:", eventCode);
  }
  ...
  console.log("[API /events/create] Upserting app_state with new active_event_code");
  ...
  console.log("[API /events/create] Broadcasting EVENT_STARTED with payload:", payload);
  ...
  console.log("[API /events/create] Successfully created event and broadcasted");
